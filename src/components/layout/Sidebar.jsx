...
export const Sidebar = ({ onLessonSelect, currentLessonId, onClose }) => {
  ...
  return (
    <aside className="w-72 flex flex-col h-full flex-shrink-0 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800">

      {/* Sidebar header */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-800">
        <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Lessons</span>
        {onClose && (
          <button
            onClick={onClose}
            aria-label="Close"
          ...
          return (
            <div key={level}>
              ...
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors hover:bg-slate-100 dark:hover:bg-slate-800 group"
              >
                <span className="text-base leading-none">{LEVEL_ICONS[level]}</span>
                <span className="flex-1 text-sm font-semibold text-slate-700 dark:text-slate-200 group-hover:text-slate-900 dark:group-hover:text-white">
                  {LEVEL_NAMES[level]}
                </span>
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg text-left text-xs transition-all ${
                          active
                            ? 'bg-brand-600 text-white font-semibold shadow-card'
      ...
      <div className="px-3 py-4 border-t border-slate-200 dark:border-slate-800 space-y-1">
        <button
          ...
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition"
        >
          <Gamepad2 className="w-4 h-4 text-violet-500 dark:text-violet-400" />
          Games &amp; Arcade
        </button>
        <button
